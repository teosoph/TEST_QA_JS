...
describe.skip(`google.com & webdriver.io`, () => {
  it.skip("should switch to another window", async () => {
    ...
    const titleGoogle = await browser.getTitle();

    await browser.pause(3000);

    // create new window
    await browser.newWindow("https://webdriver.io");
    const titleWebdriver = await browser.getTitle();
    await browser.pause(3000);

    // switch back via url match
    await browser.switchWindow(titleGoogle);
    await browser.pause(3000);

    // switch back via url match
    await browser.switchWindow(titleWebdriver);
    await browser.pause(3000);
  });

  it.only("should wait until text has changed", async () => {
    await browser.url("https://webdriver.io");
    await browser.pause(3000);
  });
});
